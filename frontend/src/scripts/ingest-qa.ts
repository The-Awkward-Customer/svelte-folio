...
async function ingestQAContent() {
  ...
  try {
    ...
    const files = readdirSync(dataSetsPath).filter((file) =>
      file.endsWith('.json')
    );

    console.log(
      `📂 Found ${files.length} JSON files to process: ${files.join(', ')}\n`
    );
    ...
    for (const [category, questions] of Object.entries(content)) {
      ...
      for (const qa of questions as any[]) {
        try {
          ...
          // Insert Q&A pair
          const qaPair = await insertQAPair(qa.q, qa.a, category, qa.tags);
          ...
          await insertEmbedding(qaPair.id, qa.q, embedding, 'question');
