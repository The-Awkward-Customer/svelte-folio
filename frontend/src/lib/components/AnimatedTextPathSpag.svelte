...
<script lang="ts">
	...
	export let colorPalette: string[] = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7'];
	...
	function createPathConfigs(height: number): PathConfig[] {
		...
		for (let i = 0; i < pathCount; i++) {
			...
			const pathSpeed =
				speedVariationRange[0] + Math.random() * (speedVariationRange[1] - speedVariationRange[0]);
			...
			const pathVerticalBounds = minBounds + Math.random() * (maxBounds - minBounds);
	...
	function generateSinglePath(width: number, height: number, config: PathConfig): PathData {
	...
	function createPathSegments() {
		...
		for (let pathIndex = 0; pathIndex < pathsData.length; pathIndex++) {
			...
			const pathIntersectionsForPath = pathIntersections.filter(
				(intersection) =>
					intersection.path1Index === pathIndex || intersection.path2Index === pathIndex
			);
			...
			for (let i = 0; i <= sortedIntersections.length; i++) {
				...
				// Create segment
				const segmentPoints = getPathPointsBetweenDistances(pathData, segmentStart, segmentEnd);
				...
				pathSegments.push({
					...
					intersectionEnd: i < sortedIntersections.length ? sortedIntersections[i] : undefined
				});
	...
	function renderMultiplePaths(progress: number) {
		...
		const sortedSegments = [...pathSegments].sort((a, b) => a.renderOrder - b.renderOrder);
	...
	function renderPathText(pathData: PathData, progress: number, centerOffset: number) {
		// Set text style for this path
		ctx.font = pathData.config.textStyle.font;
		ctx.fillStyle = pathData.config.textStyle.color;
		ctx.textAlign = 'center';
	...
	function getPointAtDistanceForPath(
		...
		const smoothAngle = getSmoothedAngleForPath(pathData, pathData.points.length - 2, 1);
		return { ...lastPoint, angle: smoothAngle };
	}
	...
	function setup() {
		...
		if (pathCount === 1) {
			...
			const avgDuration =
				pathsData.reduce((sum, path) => sum + path.length / path.config.speed, 0) /
				pathsData.length;
...
<div class="animated-text-path-container" bind:this={container}>
	<div class="canvas-wrapper">
		<canvas bind:this={canvas} class="animated-text-path" aria-hidden="true"></canvas>
	</div>
