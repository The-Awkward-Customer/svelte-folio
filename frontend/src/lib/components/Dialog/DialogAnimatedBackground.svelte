  ...
  onMount(() => {
    ...
    const animate = () => {
      ...
      circles.forEach((circle) => {
        ...
        const scale =
          Math.cos(Date.now() * 0.001 + circle.baseRadius) * 0.2 + 0.8; // Scale oscillation
        ...
        circle.y += circle.dy;
        if (
          circle.x + scaledRadius > canvas.width ||
          circle.x - scaledRadius < 0
        ) {
          circle.dx = -circle.dx;
        }
        if (
          circle.y + scaledRadius > canvas.height ||
          circle.y - scaledRadius < 0
        ) {
          circle.dy = -circle.dy;
        }
      });
...
<div class="background-container">
  <canvas bind:this={canvas} style="display: block; width: 120%; height: 100%;"
  ></canvas>
</div>
