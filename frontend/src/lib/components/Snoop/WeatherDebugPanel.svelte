<script lang="ts">
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    panelPosition.x = Math.max(
      0,
      Math.min(panelPosition.x, window.innerWidth - 320)
    );
    panelPosition.y = Math.max(
      0,
      Math.min(panelPosition.y, window.innerHeight - 400)
    );
  }
  ...
  function injectWeatherScenario(scenarioId: string) {
    const scenario = weatherDebugManager.MOCK_WEATHER_SCENARIOS.find(
      (s) => s.id === scenarioId
    );
    if (scenario) {
  ...
  function injectLocationScenario(scenarioId: string) {
    const scenario = weatherDebugManager.MOCK_LOCATION_SCENARIOS.find(
      (s) => s.id === scenarioId
    );
    if (scenario) {
            ...
            {#if debugState.forcedLoadingState}
              <div class="state-item">
                ...
                <span class="value loading"
                  >{debugState.forcedLoadingState}</span
                >
              </div>
            {/if}
            ...
            {#if debugState.mockWeatherData}
              <div class="state-item">
                ...
                <span class="value success"
                  >{debugState.mockWeatherData.name}</span
                >
              </div>
            {/if}
          </div>
        ...
        <div class="control-section">
          ...
          <div class="button-grid">
            <Button
              ...
              handleClick={() =>
                setErrorState(WeatherErrorType.LOCATION_DENIED)}
            />
            <Button
              as="button"
              label="API Error"
        ...
        <div class="control-section">
          ...
          <div class="scenario-grid">
            {#each weatherDebugManager.MOCK_WEATHER_SCENARIOS as scenario}
              <button
                class="scenario-btn"
                onclick={() => injectWeatherScenario(scenario.id)}
              >
                {scenario.name}
              </button>
            {/each}
        ...
        <div class="control-section">
          ...
          <div class="scenario-grid">
            {#each weatherDebugManager.MOCK_LOCATION_SCENARIOS as scenario}
              <button
                class="scenario-btn"
                onclick={() => injectLocationScenario(scenario.id)}
              >
                {scenario.name}
              </button>
            {/each}
