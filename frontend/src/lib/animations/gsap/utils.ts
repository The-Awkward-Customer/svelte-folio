 ...
 */
export const DEFAULT_SHUFFLE_CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*';

/**
 * Splits text into individual character spans for animation
 */
export function splitTextToSpans(element: HTMLElement, text: string): CharacterSpan[] {
 ...
 */
export function getRandomCharacter(charset: string = DEFAULT_SHUFFLE_CHARS): string {
...
export function generateShuffleSequence(
  ...
  const availableChars = charset.replace(finalChar.toUpperCase(), '').replace(finalChar.toLowerCase(), '');
