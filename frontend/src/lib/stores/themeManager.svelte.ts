...
class ThemeManager {
  ...
  private init() {
    ...
    const systemPrefers: ThemePreference = window.matchMedia(
  ...
  setActiveBrand(brand: BrandKey | null) {
    this.log.debug('setActiveBrand called', {
  ...
  private applyTheme() {
    ...
    if (this.state.brandThemeOverride) {
      ...
      this.log.debug('Applied brand-theme override', {
        brand,
        theme: themeToApply,
    ...
    // Handle legacy brand override (old system)
      ...
      root.style.setProperty(
        '--color-text-muted',
        this.adjustOpacity(textColor, 0.6)
      );
      root.style.setProperty(
        '--fg-text-muted',
        this.adjustOpacity(textColor, 0.6)
  ...
  setBrandThemeOverride(brand: BrandKey, theme: ThemePreference) {
    this.log.debug('setBrandThemeOverride called', {
      brand,
      theme,
    ...
    if (
      this.state.brandThemeOverride?.brand === brand &&
