...
function makeLogger(prefix?: string) {
  return {
    debug: (...args: unknown[]) => {
      if (shouldLog('debug')) console.debug(prefix ? `[${prefix}]` : undefined, ...args);
    },
    info: (...args: unknown[]) => {
      if (shouldLog('info')) console.info(prefix ? `[${prefix}]` : undefined, ...args);
    },
    warn: (...args: unknown[]) => {
      if (shouldLog('warn')) console.warn(prefix ? `[${prefix}]` : undefined, ...args);
    },
    error: (...args: unknown[]) => {
      if (shouldLog('error')) console.error(prefix ? `[${prefix}]` : undefined, ...args);
    }
  };
}
